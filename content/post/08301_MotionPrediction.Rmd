...
* `time_winner`：本次比赛胜者到达终点所需时间，单位为秒。

在序列中只给出了胜者到达终点所需时间，所以需要注意的有两点：

1. 需要根据`time_winner`和对应的胜者来计算胜者的移动速度。
1. 输者到达终点所需时间遵循以下规则：
    * 第一个试次输者到达终点所需时间为第一个试次的`time_winner`加上配置参数`timeInit`（默认为`500`，单位毫秒）的值，注意单位。此处的`timeInit`即为输者与胜者的时间差。
