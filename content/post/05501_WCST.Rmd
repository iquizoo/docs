...
正式测评时候，**不会**提示当前匹配特征是什么，只需要直接反馈用户每次选择是否正确即可——这样用户就要根据这个反馈来确定当前匹配特征。每当用户在当前匹配特征下累计正确完成6次后，**提示**用户当前匹配特征已改变（提示显示时间为2秒）。提示后若用户还按原来的规则判断，则记一次**固着错误**（perservative error）；用户不用原来规则答题时，若用户的选择没有合适的新规则（颜色、形状或数目中的一个），则记一次**普通的错误**；直到用户的选择**既不是原来的规则也有合适的新规则**时，判断用户正确，并把这个新规则作为下一个规则。这个新规则累计正确完成6次后，重复上面的步骤。当用户正确完成的六轮不同的规则，或者所有的48张卡片已经用完时，测评结束。
