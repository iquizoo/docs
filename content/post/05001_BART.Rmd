...
**实际测评**。实际测评时会保证气球个数（`quantity`）是最大打气次数（`maxBlow`）的倍数。例如最大打气次数为10，气球个数设置为10，20，30，……。假如这个倍数是 $M$ ，则 $M = \frac{quantity}{maxBlow}$ 。测评时候随机设定每个气球的爆炸次数为1，2，3，……，`maxBlow`，且每种爆炸次数的气球个数都为 $M$ 个。
