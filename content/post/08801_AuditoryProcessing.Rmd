...
1. 任务开始。开始任务500ms后分别呈现1s的声音刺激（升调或降调随机）两个，两个声音都结束后让用户根据听到的声音按顺序点击相应按钮。当用户正确反应后提高难度，表现为每次缩短声音刺激100ms；当呈现时间为1s但失败时难度不变；当用户失败时降低难度，表现为每次增长声音刺激100ms。当用户累计完成8个round时（见下文），任务结束。
...
【需要麻烦你们使用软件制作1s的声调和降调声音和压缩声音，包括原声合共20个刺激材料，为了管理方便，升调（up）1s的称作u1000,900ms的作u900，降调（down）的为d1000，d900如此类推】
